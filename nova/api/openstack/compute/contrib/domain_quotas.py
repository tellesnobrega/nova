...
class DomainQuotaSetsController(wsgi.Controller):
    ...
    def _validate_quota_limit(self, key, limit, minimum, maximum):
        ...
        if limit < -1:
            msg = (_("Quota limit for resource %(key)s must "
                            "be -1 or greater") % {'key': key})
            raise webob.exc.HTTPBadRequest(explanation=msg)
        if ((limit < minimum) and
           (maximum != -1 or (maximum == -1 and limit != -1))):
            msg = (_("Quota limit for resource %(key)s "
                     "must be greater than or equal to %(minimum)s") %
                     {'key': key, 'minimum': minimum})
            raise webob.exc.HTTPBadRequest(explanation=msg)
        if maximum != -1 and limit > maximum:
            msg = (_("Quota limit for resource %(key)s "
                    "must be less than or equal to %(maximum)s") %
                    {'key': key, 'maximum': maximum})
            raise webob.exc.HTTPBadRequest(explanation=msg)

    def _get_quotas(self, context, id, user_id=None, project_id=None,
                                                usages=False):
        if user_id == None and project_id == None:
            values = QUOTAS.get_domain_quotas(context, id, usages=usages)
        elif user_id:
            values = QUOTAS.get_domain_user_quotas(context, id, project_id,
                                                   user_id, usages=usages)
        else:
            values = QUOTAS.get_domain_project_quotas(context, project_id,
                                                      usages=usages)
    ...
    @wsgi.serializers(xml=DomainQuotaTemplate)
    def show(self, req, id):
        ...
        try:
            if user_id == None and project_id == None:
                ...
                return self._format_quota_set(id, self._get_quotas(context,
                                                id))
            elif user_id:
                nova.context.authorize_project_user_context(context,
                                                project_id, user_id)
                return self._format_quota_set(user_id,
                        self._get_quotas(context, id, user_id=user_id,
                                            project_id=project_id))
            else:
                nova.context.authorize_domain_project_context(context,
                                                id, project_id)
                return self._format_quota_set(project_id,
                        self._get_quotas(context, id, project_id=project_id))
    ...
    def update(self, req, id, body):
        ...
        try:
            if user_id == None and project_id == None:
                nova.context.authorize_domain_context(context, id)
                settable_quotas = QUOTAS.get_settable_quotas_for_domain(
                                                       context, id)
            elif user_id:
                nova.context.authorize_project_user_context(context,
                                                    project_id, user_id)
                settable_quotas = QUOTAS.get_settable_quotas_for_domain(
                                        context, id, project_id=project_id,
                                        user_id=user_id)
                if len(settable_quotas) <= 1:
                    # because dict may have project_id as one key
                    msg = _("The quotas for the project are not set. So, "
                            "first define the quotas for the project")
                    raise webob.exc.HTTPBadRequest(explanation=msg)
            else:
                nova.context.authorize_domain_project_context(context,
                                                       id, project_id)
                settable_quotas = QUOTAS.get_settable_quotas_for_domain(
                                    context, id, project_id=project_id)
                if len(settable_quotas) <= 1:
                    # because dict may have domain_id as one key
                    msg = _("The quotas for the domain of this project "
                            "are not set. So, first define the quotas "
                            "for the domain")
                    raise webob.exc.HTTPBadRequest(explanation=msg)
        ...
        for key, value in quota_set.items():
            ...
            if key in parNode_notFoundKeys:
                excepMessage = (_("The quota value for resource %(key)s "
    ...
    def defaults(self, req, id):
        ...
        return self._format_quota_set(id,
                    QUOTAS.get_defaults_with_domain_driver(context))
    ...
    def delete(self, req, id):
        ...
        try:
            if user_id == None and project_id == None:
                ...
                nova.context.authorize_project_user_context(context,
                                        project_id, user_id)
                QUOTAS.destroy_all_by_project_and_user_in_domain(context,
                                        project_id, user_id)
            else:
                nova.context.authorize_domain_project_context(context, id,
                                        project_id)
...
class Domain_quotas(extensions.ExtensionDescriptor):
    ...
    namespace = ("http://docs.openstack.org/compute/ext/os-domain-quota-sets/"
                 "api/v1.1")
    updated = "2014-02-27T00:00:00+00:00"
